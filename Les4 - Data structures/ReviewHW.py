...
# Задание 4. Создайте переменную text, значение которой определяется с помощью ввода данных
# с клавиатуры. Создайте словарь, в котором будет храниться слово, как ключ и сколько раз это
# слово встретилось в тексте как значение. Спрашивайте у пользователя, о каком слове он хочет
# узнать, пока пользователь не напишет: “Хватит!”
text = input("Enter any text:")
data = {}
signs = [' ', '-', ':', ',', '.', ';', '?', '!', '_', '-', "'", '"', '/', '&', '$', '@', '+']
word = ''
...
while True:
    ...
    if other_word in data:
        print(data[other_word])
    elif other_word == "хватит!":
        break
    else:
        print(0)